...
[![Conventional Commits](https://img.shields.io/badge/conventional_commits-compliant-pink?style=flat-square&logo=conventionalcommits&logoColor=white)](https://www.conventionalcommits.org/en/v1.3.0/)
...
[app-gen tool](https://github.com/mendersoftware/app-update-module/tree/1.3.0/gen), but with
...
```bash
wget -qO - https://raw.githubusercontent.com/rcwbr/mender-docker-lifecycle-helper/refs/tags/1.3.0/mender-docker-lifecycle-helper-launcher | bash -s -- --help
```
...
```bash
wget -qO - https://raw.githubusercontent.com/rcwbr/mender-docker-lifecycle-helper/refs/tags/1.3.0/mender-docker-lifecycle-helper-setup >mender-docker-lifecycle-helper && bash mender-docker-lifecycle-helper setup
```
...
docker run \
	...
	ghcr.io/rcwbr/mender-docker-lifecycle-helper:1.3.0 \
	--help
```
...
| `-w`, `--wait-for-deploy`   | `True`                                                                                                        | Wait for deployment to complete and only cache metadata on success. Requires `--device-group` to be set.                                                                                     |
...
jobs:
  mender-docker-lifecycle-helper:
    uses: 
      rcwbr/mender-docker-lifecycle-helper/.github/workflows/mender-docker-lifecycle-helper.yaml@1.3.0
    with:
...
jobs:
  mender-docker-lifecycle-helper:
    uses: rcwbr/mender-docker-lifecycle-helper/.github/workflows/mender-docker-lifecycle-helper.yaml@1.3.0
    with:
...
| `helper-image`              | ✗        | `'ghcr.io/rcwbr/mender-docker-lifecycle-helper:1.3.0'` | string | Docker image to use as mender-docker-lifecycle-helper.                                                                               |
...
[conventional commit messages](https://www.conventionalcommits.org/en/v1.3.0/) as the standard for
