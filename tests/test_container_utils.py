...
def test_split_image_ref_registry_image_tag():
...
def test_split_image_ref_registry_image_only():
...
def test_split_image_ref_digest_only():
    ...
    registry, tag, hash_val = _split_image_ref("my-registry.com/my-image@sha256:1234567890abcdef")
    assert registry == "my-registry.com/my-image"
    assert tag is None
    assert hash_val == "1234567890abcdef"


def test_split_image_ref_registry_image_tag_and_digest():
    """Test _split_image_ref with registry/image:tag and digest."""
    registry, tag, hash_val = _split_image_ref("my-registry.com/my-image:latest@sha256:1234567890abcdef")
    assert registry == "my-registry.com/my-image"
    assert tag == "latest"
    assert hash_val == "1234567890abcdef"


def test_split_image_ref_image_tag_only():
...
def test_split_image_ref_image_only():
...
def test_split_image_ref_image_digest_only():
...
def test_split_image_ref_registry_port_with_tag():
    """Test _split_image_ref with registry port and tag."""
    registry, tag, hash_val = _split_image_ref("my-registry.com:5000/my-image:latest")
    assert registry == "my-registry.com:5000/my-image"
    assert tag == "latest"
    assert hash_val is None


def test_split_image_ref_registry_port_with_digest():
    """Test _split_image_ref with registry port and digest."""
    registry, tag, hash_val = _split_image_ref("my-registry.com:5000/my-image@sha256:1234567890abcdef")
    assert registry == "my-registry.com:5000/my-image"
    assert tag is None
    assert hash_val == "1234567890abcdef"


def test_split_image_ref_registry_port_with_tag_and_digest():
    """Test _split_image_ref with registry port, tag, and digest."""
    registry, tag, hash_val = _split_image_ref("my-registry.com:5000/my-image:latest@sha256:1234567890abcdef")
    assert registry == "my-registry.com:5000/my-image"
    assert tag == "latest"
    assert hash_val == "1234567890abcdef"


def test_image_ref_hash_or_tag_with_hash():
    """Test _image_ref_hash_or_tag with tag and hash provided (should prefer hash)."""
    result = _image_ref_hash_or_tag("my-registry.com/my-image", "latest", "1234567890abcdef")
...
def test_image_ref_hash_or_tag_with_hash():
    ...
    result = _image_ref_hash_or_tag("my-registry.com/my-image", None, "1234567890abcdef")
    assert result == "my-registry.com/my-image@sha256:1234567890abcdef"


def test_image_ref_hash_or_tag_with_tag():
...
def test_image_ref_hash_or_tag_neither():
...
def test_image_ref_hash_or_tag_empty_registry_with_tag():
    """Test _image_ref_hash_or_tag with empty registry and tag provided."""
    result = _image_ref_hash_or_tag("", "latest", None)
    assert result == ":latest"


def test_image_ref_hash_or_tag_empty_registry_with_hash():
...
def test_save_registry_image_to_file_success(tmp_path):
...
def test_save_registry_image_to_file_not_found(tmp_path):
...
def test_save_registry_image_to_file_other_error(tmp_path):
...
def test_save_local_image_to_file_success(tmp_path):
...
def test_save_local_image_to_file_not_found(tmp_path):
    ...
    with patch("subprocess.run") as mock_run:
        ...
        assert "Hash 1234567890abcdef not found in local daemon." == str(exc_info.value)


def test_save_local_image_to_file_other_error(tmp_path):
    with patch("subprocess.run") as mock_run:
        error = subprocess.CalledProcessError(1, "skopeo")
...
def test_save_image_to_file_hash_mismatch(tmp_path):
    ...
    assert "Specified hash 1234567890abcdef does not match hash embedded in ref my-image@sha256:wronghash." == str(exc_info.value)


def test_save_image_to_file_local_success(tmp_path):
    ...
    with patch("mender_docker_lifecycle_helper.utils.container_utils.save_local_image_to_file") as mock_local:
        with patch("mender_docker_lifecycle_helper.utils.container_utils.save_registry_image_to_file") as mock_registry:
            save_image_to_file(image, file_path)
...
def test_save_image_to_file_local_success_with_hash_in_ref(tmp_path):
    """Test that save_image_to_file calls save_local_image_to_file when ref already contains hash."""
    image = {
        "ref": "my-image@sha256:1234567890abcdef",
        "hash": "1234567890abcdef"
    }
    file_path = tmp_path / "image.tar"
...
def test_save_image_to_file_registry_fallback_success(tmp_path):
    ...
    with patch("mender_docker_lifecycle_helper.utils.container_utils.save_local_image_to_file") as mock_local:
        with patch("mender_docker_lifecycle_helper.utils.container_utils.save_registry_image_to_file") as mock_registry:
            ...
            mock_registry.assert_called_once_with("my-image@sha256:1234567890abcdef", file_path)


def test_save_image_to_file_registry_fallback_success_hash_in_ref(tmp_path):
...
def test_save_image_to_file_both_fail(tmp_path):
    ...
    with patch("mender_docker_lifecycle_helper.utils.container_utils.save_local_image_to_file") as mock_local:
        with patch("mender_docker_lifecycle_helper.utils.container_utils.save_registry_image_to_file") as mock_registry:
            ...
            assert str(exc_info.value) == "Image with ref my-image@sha256:1234567890abcdef not found in local daemon or remote registry"


def test_save_image_to_file_propagates_other_exceptions_from_local(tmp_path):
...
def test_save_image_to_file_propagates_other_exceptions_from_registry(tmp_path):
...
def test_save_registry_image_to_file_integration(tmp_path):
...
def test_save_image_to_file_integration(tmp_path):
    ...
    assert (extract_dir / "index.json").read_text() == '{"schemaVersion":2,"mediaType":"application/vnd.oci.image.index.v1+json","manifests":[{"mediaType":"application/vnd.oci.image.manifest.v1+json","digest":"sha256:298efc24641ff8a1a285abdc555a0ce5ab7c42eb085e1be099f824188e069604","size":608}]}'
    assert (extract_dir / "oci-layout").read_text() == '{"imageLayoutVersion":"1.0.0"}'
    assert (extract_dir / "blobs" / "sha256" / "0188a8de47ca89b720586f01da7d7f870bdcf5f770b19f740291d716235d3107").exists()
    assert (extract_dir / "blobs" / "sha256" / "298efc24641ff8a1a285abdc555a0ce5ab7c42eb085e1be099f824188e069604").exists()
    assert (extract_dir / "blobs" / "sha256" / "5bfa213ad2917fd7ef0d56c49b841de3a4788e60b3554ad86d616100e095c1f8").exists()
