...
from mender_docker_lifecycle_helper.utils.image_cache import ImageCache, ImageDirFormatException
...
class TestImageCacheDelta:
    ...
    def test_delta_existing_delta_returns_path(self, tmp_path):
        ...
        result = cache.delta({"ref": "from-image", "hash": "hash1"}, {"ref": "to-image", "hash": "hash2"})
    ...
    def test_delta_mock_oci_deep_delta(self, tmp_path):
        ...
        from_tar = _create_oci_tar(tmp_path, image_name="from-image", digest="from_hash")
        ...
        with patch('mender_docker_lifecycle_helper.utils.image_cache.oci_deep_delta') as mock_oci_deep_delta:
            mock_return_path = tmp_path / "delta" / "from_hash" / "to_hash" / "image.img"
            mock_oci_deep_delta.return_value = mock_return_path

            # Call delta method
            result = cache.delta({"ref": "from-image", "hash": "from_hash"}, {"ref": "to-image", "hash": "to_hash"})
    ...
    def test_delta_cached_does_not_call_oci_deep_delta(self, tmp_path):
        ...
        with patch('mender_docker_lifecycle_helper.utils.image_cache.oci_deep_delta', return_value=mock_return_path) as mock_oci:
            # First call should invoke oci_deep_delta
            result1 = cache.delta({"ref": "from-image", "hash": "from_hash"}, {"ref": "to-image", "hash": "to_hash"})
            ...
            result2 = cache.delta({"ref": "from-image", "hash": "from_hash"}, {"ref": "to-image", "hash": "to_hash"})
...
class TestImageCacheExtractCacheFile:
    ...
    def test_extract_cache_file_success(self, tmp_path):
        ...
        tar_path = _create_oci_tar(tmp_path, image_name="test-image", digest="sha256abc123")
    ...
    def test_extract_cache_file_missing_oci_layout_raises(self, tmp_path):
        ...
        with tarfile.open(tar_path, "w") as tar:
            ...
            index_content = json.dumps({
                "schemaVersion": 2,
                "manifests": [{"digest": "sha256abc", "annotations": {"io.containerd.image.name": "test"}}]
            }).encode("utf-8")
            index_info = tarfile.TarInfo(name="index.json")
            index_info.size = len(index_content)
            tar.addfile(index_info, io.BytesIO(index_content))
    ...
    def test_extract_cache_file_missing_ref_raises(self, tmp_path):
        ...
        assert "does not contain expected io.containerd.image.name metadata" in str(exc_info.value)
    ...
    def test_extract_cache_file_missing_digest_raises(self, tmp_path):
        ...
        with tarfile.open(tar_path, "w") as tar:
            ...
            index_content = json.dumps({
                "schemaVersion": 2,
                "manifests": [{"annotations": {"io.containerd.image.name": "test"}}]  # No digest
            }).encode("utf-8")
            index_info = tarfile.TarInfo(name="index.json")
            index_info.size = len(index_content)
            tar.addfile(index_info, io.BytesIO(index_content))
    ...
    def test_extract_cache_file_populates_extract_cache(self, tmp_path):
        ...
        tar_path = _create_oci_tar(tmp_path, image_name="test-image", digest="sha256hash1")
    ...
    def test_extract_cache_file_content(self, tmp_path):
        ...
        tar_path = _create_oci_tar(
            tmp_path,
            image_name="content-test-image",
            digest="sha256content123"
        )
        ...
        assert manifest_section["mediaType"] == "application/vnd.oci.image.manifest.v1+json"
        assert manifest_section["size"] == 42
        assert manifest_section["annotations"]["io.containerd.image.name"] == "content-test-image"
...
class TestImageCacheExtractCacheImage:
    ...
    def test_extract_cache_image_not_in_cache(self, tmp_path):
        ...
        tar_path = _create_oci_tar(tmp_path, image_name="test-image", digest="sha256hash2")
        cache = ImageCache(tmp_path)
    ...
    def test_extract_cache_image_not_in_either_cache(self, tmp_path, monkeypatch):
        ...
        tar_path = _create_oci_tar(tmp_path, image_name="test-image", digest="sha256hash3")
        ...
        monkeypatch.setattr("mender_docker_lifecycle_helper.utils.image_cache.save_image_to_file", mock_save_image_to_file)
...
class TestImageCacheSaveCacheImage:
    ...
    def test_save_cache_image_new_image(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("mender_docker_lifecycle_helper.utils.image_cache.save_image_to_file", fake_save_image_to_file)

        tar_path = _create_oci_tar(tmp_path, image_name="test-image", digest="sha256oldhash")
        cache = ImageCache(tmp_path)
    ...
    def test_save_cache_image_existing_image_updates_timestamp(self, tmp_path):
        ...
        tar_path = _create_oci_tar(tmp_path, image_name="test-image", digest="sha256touchhash")
        cache = ImageCache(tmp_path)
        ...
        assert final_mtime > initial_mtime, "File timestamp should be updated for existing image"
...
class TestImageCacheExtractOciFile:
    ...
    def test_extract_oci_file_uses_filter_tar(self, tmp_path, monkeypatch):
        ...
        def mock_extractall(self, path, members=None, *, numeric_owner=False, **kwargs):
            ...
            return original_extractall(self, path, members, numeric_owner=numeric_owner, filter="tar")
