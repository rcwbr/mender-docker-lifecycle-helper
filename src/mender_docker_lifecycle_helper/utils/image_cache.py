...
class ImageCache:
    ...
    def __init__(
        ...
        self.delta_cache = {
            from_dir.name: {
                ...
                for to_dir in from_dir.iterdir() if to_dir.is_dir()
            }
            for from_dir in self.delta_cache_dir.iterdir() if from_dir.is_dir()
        }
        self.extract_cache_dir = (cache_dir / extract_cache_dirname)
        self.extract_cache_dir.mkdir(exist_ok=True)
        self.extract_cache = {
            hash_dir.name: hash_dir
            for hash_dir in self.extract_cache_dir.iterdir() if hash_dir.is_dir()
        }
        ...
        self.save_cache = {
            ...
            for hash_dir in self.save_cache_dir.iterdir() if hash_dir.is_dir()
        }


    def delta(
        self,
        from_image: dict[str, str],
        to_image: dict[str, str]
    ) -> Path:
        """
        ...
        if to_hash in self.delta_cache.get(from_hash, {}):
            ...
            self.logger.debug(f"Found cached delta file {delta_file} for from hash {from_hash} to hash {to_hash}.")
            # Update the file timestamp for cache cleanup logic
            delta_file.touch()
            return delta_file
        else:
            delta_dir = (self.delta_cache_dir / from_hash / to_hash)
            self.logger.debug(f"Creating delta for {from_hash} to {to_hash} under {delta_dir}...")
    ...
    def _extract_oci_file(self, extract_dir: Path, extract_file: Path) -> None:
        ...
        if not (extract_dir / OCI_LAYOUT_FILENAME).exists():
            raise ImageDirFormatException(f"{extract_file} as extracted to {extract_dir} is not in valid OCI format.")
    ...
    def extract_cache_file(self, extract_file: Path) -> dict[str, str]:
        ...
        image_ref = ""
        image_hash = ""
        with tempfile.TemporaryDirectory(dir=self.extract_cache_dir) as temp_extract_dir:
            temp_extract_dir = Path(temp_extract_dir)
            self.logger.debug(f"Extracting {extract_file} into temp dir {temp_extract_dir}")
            ...
            image_ref = image_manifest.get("annotations", {}).get("io.containerd.image.name", None)
            image_hash = image_manifest.get("digest", None)
            if image_ref is None:
                raise ImageDirFormatException(f"{extract_file} as extracted to {temp_extract_dir} does not contain expected io.containerd.image.name metadata in its index.")
            if image_hash is None:
                raise ImageDirFormatException(f"{extract_file} as extracted to {temp_extract_dir} does not contain digest metadata in its index.")

            image_extract_cache_dir = self.extract_cache_dir / image_hash
            self.logger.debug(f"Saving extracted image contents from {extract_file} to cache {image_extract_cache_dir}")
        ...
        self.logger.debug(f"Saving image file {extract_file} to cache {image_save_cache_file}")
    ...
    def extract_cache_image(self, image: dict[str, str]) -> Path:
        ...
        if image_hash in self.extract_cache:
            self.logger.debug(f"Image {image['ref']} with hash {image_hash} already saved in cache at {self.extract_cache[image_hash]}.")
            return self.extract_cache[image_hash]

        image_file = self.save_cache_image(image)
        self.logger.debug(f"Extracting image file {image_file} with hash {image_hash} to cache.")
    ...
    def save_cache_image(self, image: dict[str, str]) -> Path:
        ...
        if image_hash in self.save_cache:
            ...
            self.logger.debug(f"Image with hash {image_hash} already saved in cache at {save_image_file}.")
            # Update the file timestamp for cache cleanup logic
            save_image_file.touch()
            return save_image_file
        else:
            save_image_dir = self.save_cache_dir / image_hash
            self.logger.debug(f"Saving image with hash {image_hash} for ref {image['ref']} to cache at {save_image_dir}.")
