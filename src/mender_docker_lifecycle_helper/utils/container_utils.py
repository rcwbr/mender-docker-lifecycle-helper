...
HASH_NOT_FOUND_DAEMON_LOG = "loading image from docker engine: Error response from daemon: failed to get digest"
...
def get_image_hash(
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"docker inspect failed for image {image_ref}: {e}\n{e.stderr}")
            raise ValueError(f"Could not retrieve hash for image: {image_ref}")

    return image_hash


def save_registry_image_to_file(image_ref: str, file: Path) -> subprocess.CompletedProcess:
    """
    ...
    try:
        result = subprocess.run(
            [SKOPEO_BIN, "copy", f"{REGISTRY_TRANSPORT}{image_ref}", f"{OCI_TRANSPORT}:{file}"],
...
def save_local_image_to_file(image_hash: str, file: Path) -> subprocess.CompletedProcess:
    """
    ...
    try:
        result = subprocess.run(
            [SKOPEO_BIN, "copy", f"{DAEMON_TRANSPORT}{HASH_PREFIX}:{image_hash}", f"{OCI_TRANSPORT}:{file}"],
            capture_output=True, check=True
        )
        if result.returncode != 0:
            raise subprocess.SubprocessError(result.stdout, result.stderr)
        ...
        if (
            ...
            raise ImageNotFoundException(f"Hash {image_hash} not found in local daemon.")
        else:
            raise e


def save_image_to_file(image: dict[str, str], file: Path) -> subprocess.CompletedProcess:
    """
    ...
    if (
        image_ref_hash is not None
        and image_ref_hash != image_hash
    ):
