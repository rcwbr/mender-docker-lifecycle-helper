...
class LifecycleHelperArtifact:
    ...
    def __init__(
        ...
        self.image_ids = [
            service_spec["image"]["hash"] for service_spec in artifact_metadata.services.values()
        ]
    ...
    def prep_delta_image(
        ...
        previous_image = self.context.previous_artifact_metadata.services[service_name]["image"]
        # If the image ref and hash for a service match those in the previous artifact metadata, include a dummy delta file in the artifact
        if (previous_image == service_image):
            self.context.logger.debug(f"Image {service_image['ref']} with hash {service_image['hash']} for service {service_name} matches the previous artifact. Will include an empty delta for this image.")
            # TODO prepare empty delta file for artifact
            ...
            (artifact_image_prep_dir / "sums-current.txt").write_text(previous_image["hash"])
    ...
    def prep_image(
        ...
        self.context.logger.debug(f"Preparing artifact files for image {service_image['ref']} for service {service_name} in {artifact_image_prep_dir}")
        ...
        if (
            ...
            try:
                ...
                self.context.logger.warning(f"Will include the full image {service_image} for {service_name} instead.")
        ...
        self.context.logger.debug(f"Including full image {service_image['ref']} with hash {service_image['hash']} for service {service_name} in artifact.")
    ...
    def prep_images(
        ...
        self.context.logger.debug(f"Preparing images artifact files in {artifact_images_prep_dir}")

        artifact_images_prep_dir.mkdir()
        for service_name, service_spec in self.artifact_metadata.services.items():
            self.prep_image(service_name, service_spec["image"], artifact_images_prep_dir)
        ...
        self.context.logger.debug(f"Cleaning up images prep dir {artifact_images_prep_dir}")
    ...
    def prep_manifests(
        ...
        self.context.logger.debug(f"Preparing manifests artifact files in {artifact_manifests_prep_dir}")

        artifact_manifests_prep_dir.mkdir()
        artifact_manifest = artifact_manifests_prep_dir / self.context.manifest_file.name
        ...
        self.context.logger.debug(f"Cleaning up manifests prep dir {artifact_manifests_prep_dir}")
    ...
    def gen_artifact_file(self, artifact_file: Path) -> None:
        """
        Prepare the artifact by generating necessary files and processing via the mender-artifact tool.

        :param artifact_file: The file at which to generate the artifact.

        :returns: None
        """
        artifact_prep_dir = self.context.cache_dir / "artifact_prep"
    ...
    @staticmethod
    def gen_artifact_services(context):
        """
        Determine the services metadata for the current artifact, including reading the image hashes from remote images when required. To establish this list, any provided service-file image archives are extracted and read. Returns a dict of service names to image {ref, hash} dicts.
        ...
        for service_name, service_spec in context.manifest["services"].items():
            ...
            if service_name in context.service_files:
                if service_name in context.service_images:
                    context.logger.warning(f"Service {service_name} has both a service file and an image override specified. Ignoring the image override.")
                ...
                context.logger.debug(f"Overriding {service_name} image details from file {service_filename} to ref {service_image['ref']} and hash {service_image['hash']}.")
                artifact_service["ref"] = service_image["ref"]
                artifact_service["hash"] = service_image["hash"]
            ...
            # The metadata from each specified image override is used in the artifact
                ...
                context.logger.debug(f"Overriding {service_name} image ref to {image_override}.")
                artifact_service["ref"] = image_override
                artifact_service["hash"] = context.match_or_find_hash(service_name, image_override)

            # By default, the image ref from the manifest is used for the artifact
            else:
                artifact_service["ref"] = service_spec["image"]
                artifact_service["hash"] = context.match_or_find_hash(service_name, service_spec["image"])
        ...
        for service_name, service_filename in context.service_files.items():
            if service_name not in context.manifest["services"]:
                ...
                raise ManifestContentMismatchException(f"Service {service_name} specified to map to service file {service_filename} not found in manifest.")

        # Check that all service image args match the manifest
        for service_name, image_override in context.service_images.items():
            if service_name not in context.manifest["services"]:
                # Cannot map specified service name to service in the manifest
                raise ManifestContentMismatchException(f"Service {service_name} specified to map to service image override {image_override} not found in manifest.")
